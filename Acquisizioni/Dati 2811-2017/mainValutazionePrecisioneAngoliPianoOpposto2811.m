...
%% PIANO SAGITTALE CON MOVIMENTO PIANO FRONTALE
%%
piano = 'F';
...
for j = 1:nAngoli
    for i = 1:nAcqu
        ...
        datiSag.(['deg' angoliFerrari{j}]) = [datiSag.(['deg' angoliFerrari{j}]); struttura.dataHum];
...
for j = 1:nAngoli
    campoAngolo = ['deg' angoli{j}];
    subplot(3, 7, j)
    plot(ones(size(angoliCalcolatiSag.(campoAngolo).acos))*str2double(angoli{j}), 'LineWidth', 2) % plotto valore ideale
    hold on
...
legend(legin)
set(h, 'Name', 'Movimento sul Piano Frontale e osservazione Piano Sagittale')

% Salviamo i plot
savefig2(h, fullfile(pwd, 'graficiPianoOpposto', 'confrontoValutazioneAngoliSag.fig'), 'ScreenSize', false)
...
set(h2, 'Name', 'Movimento sul Piano Frontale e osservazione Piano Sagittale')

% Salviamo i plot
savefig2(h2, fullfile(pwd, 'graficiPianoOpposto', 'indiciValutazioniAngoliSag.fig'), 'ScreenSize', false)

%% PIANO FRONTALE CON MOVIMENTO PIANO SAGITTALE
%%
piano = 'S';
...
datiFront.deg0 = struttura.dataHum;
struttura = load('0_2S');
datiFront.deg0 = [datiFront.deg0; struttura.dataHum];
...
for j = 2:nAngoli 
    ...
    struttura = load(nomeFile);
    datiFront.(['deg' angoliFerrari{j}]) = struttura.dataHum;
...
for j = 1:nAngoli
    campoAngolo = ['deg' angoli{j}];
    subplot(3, 7, j)
    plot(ones(size(angoliCalcolatiFront.(campoAngolo).acos))*str2double(angoli{j}), ...
...
legend(legin)
set(h, 'Name', 'Movimento sul Piano Sagittale e osservazione Piano Frontale')

% Salviamo i plot
savefig2(h, fullfile(pwd, 'graficiPianoOpposto', 'confrontoValutazioneAngoliFront.fig'), 'ScreenSize', false)
...
set(h2, 'Name', 'Movimento sul Piano Sagittale e osservazione Piano Frontale')

% Salviamo i plot
savefig2(h2, fullfile(pwd, 'graficiPianoOpposto', 'indiciValutazioniAngoliFront.fig'), 'ScreenSize', false)